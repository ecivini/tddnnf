...
def simple_boolean_formula():
    ...
    return Or(Symbol("A", BOOL), And(Symbol("B", BOOL), Not(Symbol("C", BOOL))))
...
def test_output_file_creation_sat(sat_formula, temp_dir):
    ...
    assert os.path.isfile(tddnnf_file), "tddnnf.smt2 should be created for SAT formula"
...
def test_circuit_bc_file_creation(sat_formula, temp_dir):
    ...
    with open(circuit_file, "r", encoding="utf8") as f:
        ...
        assert first_line == "c BC-S1.2", "circuit.bc should start with BC-S1.2 header"
...
def test_mapping_files_creation(sat_formula, temp_dir):
    ...
    assert os.path.isfile(important_labels_file), "important_labels.json should be created"
...
def test_compilation_output_nnf_creation(sat_formula, temp_dir):
    ...
    with open(nnf_file, "r", encoding="utf8") as f:
        ...
        assert first_char in ["o", "t", "f", "a"], f"NNF file should start with valid node type, got '{first_char}'"
...
def test_full_workflow_with_file_output(temp_dir):
    ...
    phi = Or(LT(Symbol("X", REAL), Symbol("Y", REAL)), Equals(Symbol("X", REAL), Real(10)))
